...
DATABASE_URL = os.environ.get("DATABASE_URL", "mongodb+srv://mongoguess:guessmongo@cluster0.zcwklzz.mongodb.net/?retryWrites=true&w=majority") # MongoDB veritabanınızın url'si. Nasıl alacağınızı bilmiyorsanız destek grubu gelin.
BOT_USERNAME = os.environ.get("BOT_USERNAME","EpicGuessBot") # Botunuzun kullanıcı adı.
LOG_CHANNEL = int(os.environ.get("LOG_CHANNEL","-1001881565361")) # Botunuzun eylemleri kaydedeceği kayıt grubunun id'si.
...
async def handle_user_status(bot: Client, cmd: Message): # Kullanıcı kontrolü
    ...
    if not await db.is_user_exist(chat_id):
        if cmd.chat.type == "private":
            ...
            await db.add_user(chat_id)
            chat = await bot.get_chat(chat_id)  # Updated line
